...
%                 end
    ...
    methods (Access = private)
        ...
        function MinimumInPixelIntensitySliderValueChanging(app, event)
            ...
            if app.BrightnessSlider.Value == 0
                app.adjusted_image = imadjust(app.cropped_image,[changingValue app.MaximumInPixelIntensitySlider.Value],[]);
                app.in_out = linspace(0,1);
        ...
        function MaximumInPixelIntensitySliderValueChanging(app, event)
            changingValue = event.Value;
            if app.BrightnessSlider.Value == 0
                app.adjusted_image = imadjust(app.cropped_image,[app.MinimumInPixelIntensitySlider.Value changingValue],[]);
                app.in_out = linspace(0,1);
        ...
        function AcceptChangesButtonPushed(app, event)
            ...
            switch app.bit_d_val
                case 2^8
                    app.GelBoxApp.gel_data.image.adjusted_image = uint8(app.GelBoxApp.gel_data.image.adjusted_image);
                case 2^16
                    app.GelBoxApp.gel_data.image.adjusted_image = uint16(app.GelBoxApp.gel_data.image.adjusted_image);
            end
            ImageAdjusted(app.GelBoxApp)
            delete(app)
        end

        % Button pushed function: ReverttoOriginalImageButton
        function ReverttoOriginalImageButtonPushed(app, event)
            app.adjusted_image = [];
            app.adjusted_image_2 = [];
            app.MinimumInPixelIntensitySlider.Value = 0;
            app.MaximumInPixelIntensitySlider.Value = 1;
            ...
            app.RevertChangesButton.Enable = 'on';
        end
