function gel_box_summary3(summary,path_string,file_string)

[m,n] = size(summary);

prompt = {'Enter bottom band label:','Enter middle band label:','Enter top band label:'};

dlgtitle = 'Band Labels';
dims = [1 65];
definput = {'Bottom Band','Middle Band','Top Band'};
...
left_pads([2 3 5 6]) = 0.1;
right_pads = 0.35 * ones(1,no_of_panels_wide);
...
for i = 1 : length(im_handles)
    ...
    hold on
    bottom = patch(summary(i).band_1, ...
        summary(i).y,'b','FaceAlpha',0.1);
    middle = patch(summary(i).band_2, ...
        summary(i).y,'y','FaceAlpha',0.1);
    top = patch(summary(i).band_3, ...
        summary(i).y,'r','FaceAlpha',0.1);
    ...
    bottom_area = (summary(i).bottom)/(summary(i).top+summary(i).bottom+ ...
        summary(i).middle);
    middle_area = (summary(i).middle)/(summary(i).top+summary(i).bottom+ ...
        summary(i).middle);

    
    str_top = sprintf('%%%s = %.2f',string(labels{3}),top_area);
    str_middle = sprintf('%%%s = %.2f',string(labels{2}),middle_area);
    str_bottom = sprintf('%%%s = %.2f',string(labels{1}),bottom_area);
    leg_labels = {str_top,str_middle,str_bottom};

    legendflex([top middle bottom], leg_labels, ...
        'xscale',0.15, ...
        'anchor',{'nw','nw'}, ...
        'buffer',[60 0], ...
        'padding',[1 1 2], ...
    ...
    hold on
    fill(sp(back_fit_handles(i)),summary(i).x_back+summary(i).band_1, ...
        summary(i).y,'b','FaceAlpha',0.1);
    fill(sp(back_fit_handles(i)),summary(i).x_back+summary(i).band_2, ...
        summary(i).y,'y','FaceAlpha',0.1);
    fill(sp(back_fit_handles(i)),summary(i).x_back+summary(i).band_3, ...
        summary(i).y,'r','FaceAlpha',0.1);
