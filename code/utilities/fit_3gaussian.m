function [y_bands, y_fit,r_squared]= fit_3gaussian(x,y,y_back,no_of_bands)
...
if numel(peaks.max_indices) == no_of_bands
    ...
    mid_band_x_estimate = peaks.max_indices(2)
    upper_band_x_estimate=peaks.max_indices(3)
else
    lower_band_x_estimate = 0.3*length(x);
    mid_band_x_estimate = 0.6*length(x);
    upper_band_x_estimate = 0.9*length(x);
end
target = y';
...
% Let's find a suitable starting point for optimization
...
mid_amp_estimate = upper_amp_estimate;
...
%     plot(y_bands(2,:)+y_back,x,'bo');
    ...
    function [y_bands,y_fit] = calculate_profile(x,par)
        ...
        ygreen=skewed_Gaussian(x,x3,curve_shape1,amp3,skew1);

        y_fit=yblue+yred+ygreen;
        y_bands(1,:) = yred;
        y_bands(2,:) = yblue;
        y_bands(3,:) = ygreen;
