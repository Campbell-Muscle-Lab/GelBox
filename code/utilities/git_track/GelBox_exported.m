classdef GelBox_exported < matlab.apps.AppBase
            ...
            if (isfield(app.gel_data.boxes,'box_handle'))
                ...
                for i = disp_start:disp_end
                    ...
                    set(app.gel_data.boxes.box_label(i), ...
                        'Position',[app.d.box(i).position(1)+0.5*app.d.box(i).position(3) ...
        ...
        function BackgroundCorrection(app,x,method,box_no)
            switch char(method)
                ...
                case 'Rolling Ball (RB)'
                    ...
                    im_close = [];
                    a_im = app.d.box(box_no).inset;
                    im_close = imclose((mean(a_im,2)),se);
                    im_close = cast(im_close,class(a_im));
                    im_close = double(imcomplement(im_close));
                    app.gel_data.background(box_no).x_back = flipud(im_close);
                    app.gel_data.settings.background.method{box_no} = char(method);
    ...
    methods (Access = private)
        ...
        function LoadAnalysisButtonPushed(app, event)
            ...
            if (path_string~=0)
                ...
                for i=1:n
                    ...
                    app.gel_data.boxes.box_label(i) = text(p(1)+0.5*p(3),p(2)-50,sprintf('%.0f',i), ...
                        'Parent',app.gel_image_axis,'FontWeight',"bold","FontSize",18,'HorizontalAlignment', 'center');
        ...
        function NewBoxButtonPushed(app, event)
            ...
            app.gel_data.boxes.box_label(n) = text(app.gel_image_axis, ...
                p(1)+0.5*p(3),p(2)-50,sprintf('%.0f',n),'FontWeight',"bold","FontSize",18,'HorizontalAlignment', 'center');
        ...
        function LoadLaneLayoutMenuSelected(app, event)
            ...
            if (path_string_layout~=0)
                ...
                %     message = ["Layout does not have ""Box"" as one of the columns.","Please make sure to add ""Box"" to Excel file."];
                %     selection = uiconfirm(app.GelBoxUIFigure,message,"Reload Layout",...
                %         "Options",["Reload Layout","Skip"],"DefaultOption",1,"CancelOption",2,'Icon','error');
                % 
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.RollingBallSizeSpinner.Value = 100;
